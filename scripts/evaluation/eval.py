"""Offline evaluation helper for LabelScan-Go audit outputs."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LabelScan-Go offline evaluation helper")
...
def evaluate_risk(dbar, cv, tau_d, tau_cv, mu_d, mu_cv):
    ...
    if is_d_outlier and is_cv_outlier:
        flag = "High member risk"
        base_score = 80
    elif is_d_outlier or is_cv_outlier:
        flag = "Elevated member risk"
        base_score = 50
    else:
        flag = "Low member risk"
        base_score = 10
...
def main():
    ...
    print("LabelScan-Go evaluation helper starting...")
    print("-----------------------------------------------------")
    ...
    dummy_audit_dists = [
        {"id": 1, "dists": [3.20, 3.21, 3.19, 3.20, 3.22, 3.18, 3.20, 3.21, 3.19, 3.20, 3.22], "label_true": 0, "label_pred": 0},
        {"id": 2, "dists": [1.02, 1.05, 0.88, 1.15, 0.90, 1.10, 0.95, 1.08, 0.85, 1.12, 0.92], "label_true": 1, "label_pred": 1},
        {"id": 3, "dists": [1.50, 1.48, 1.52, 1.49, 1.51, 1.50, 1.47, 1.53, 1.48, 1.52, 1.49], "label_true": 2, "label_pred": 2}
    ]
